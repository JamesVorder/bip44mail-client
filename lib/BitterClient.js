...
BitterClient.getEmailsAtAddress = function(addr, callback){
    api.getEmails(addr, fingerprint, function(err, data){
        if(err) callback(err, undefined);
        else callback(undefined, data);
    });
};

BitterClient.signMessage = function(keystore, message, password, addr, callback){
...
BitterClient.hashMessage = function(message, callback){
     callback(undefined, util.sha3(message));
};
